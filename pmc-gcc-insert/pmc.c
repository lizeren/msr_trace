...
// ===== Function list for instrumentation =====
#define MAX_FUNCTIONS 256
#define MAX_FUNCTION_NAME_LENGTH 256

static struct {
    char functions[MAX_FUNCTIONS][MAX_FUNCTION_NAME_LENGTH];
    size_t count;
    int initialized;
} function_list = {0};

// ===== Handle stack for nested function calls =====
#define MAX_HANDLE_STACK 64

static __thread struct {
    pmc_multi_handle_t *handles[MAX_HANDLE_STACK];
    size_t depth;
} handle_stack = {0};
...
static int load_function_list_from_csv(const char *csv_path) {
    if (function_list.initialized) {
        return 0;  // Already loaded
    }
    
    const char *debug_env = getenv("PMC_DEBUG");
    if (debug_env) {
        fprintf(stderr, "PMC: Loading function list from: %s\n", csv_path);
    }
    
    FILE *fp = fopen(csv_path, "r");
    if (!fp) {
        // If file doesn't exist, that's okay - instrumentation just won't work
        if (debug_env) {
            fprintf(stderr, "PMC: Warning: Could not open function list file: %s (%s)\n", 
                    csv_path, strerror(errno));
        }
        function_list.initialized = 1;  // Mark as initialized to avoid repeated attempts
        return -1;
    }
    ...
    while (fgets(line, sizeof(line), fp) && function_list.count < MAX_FUNCTIONS) {
        ...
        if (strlen(line) > 0) {
            strncpy(function_list.functions[function_list.count], line, 
                   sizeof(function_list.functions[0]) - 1);
            function_list.functions[function_list.count][sizeof(function_list.functions[0]) - 1] = '\0';
            if (debug_env) {
                fprintf(stderr, "PMC: Added function to list: %s\n", function_list.functions[function_list.count]);
            }
            function_list.count++;
        }
    }
    ...
    fclose(fp);
    function_list.initialized = 1;
    
    if (debug_env) {
        fprintf(stderr, "PMC: Loaded %zu functions from %s\n", function_list.count, csv_path);
    }
    
    return 0;
}

static int is_function_in_list(const char *func_name) {
    if (!function_list.initialized) {
        // Try to load from default path
        const char *api_list_path = getenv("PMC_API_LIST");
        if (!api_list_path) {
            api_list_path = "api_lists.csv";
        }
        printf("load from csv\n");
        load_function_list_from_csv(api_list_path);
    }
    
    if (function_list.count == 0) {
        return 0;  // No functions in list
    }
    
    for (size_t i = 0; i < function_list.count; i++) {
        if (strcmp(function_list.functions[i], func_name) == 0) {
            printf("in the list\n");
            return 1;
        }
    }
    printf("function %s not in the list\n", func_name);
    return 0;
}
...
static pmc_multi_handle_t* pmc_measure_begin_internal(const char *label,
    ...
    // Allocate and copy label string (will be freed in pmc_measure_end)
    handle->label = strdup(label);
    if (!handle->label) {
        pmc_set_error("Failed to allocate label string");
        free(handle);
        return NULL;
    }
...
void pmc_measure_end(pmc_multi_handle_t *handle, int report) {
    ...
    if (report) {
        ...
        pmc_export_json(handle, output_file);  // Export to JSON
    }
    ...
    // Free the label string (allocated in pmc_measure_begin_internal)
    if (handle->label) {
        free((void*)handle->label);
    }
...
// ===== Instrumentation Functions for -finstrument-functions =====

/**
 * Called automatically by GCC when -finstrument-functions is used.
 * This function is called at the entry of every function.
 * 
 * @param this_fn Address of the function being entered
 * @param call_site Address of the call site (unused)
 */
void __cyg_profile_func_enter(void *this_fn, void *call_site) {
    ...
    if (!dladdr(this_fn, &info) || !info.dli_sname) {
        printf("can't resolve symbol\n");
        return;  // Can't resolve symbol, skip
    }
    
    // Debug: Check if function list is loaded
    static int debug_printed = 0;
    const char *debug_env = getenv("PMC_DEBUG");
    if (debug_env && !debug_printed) {
        fprintf(stderr, "PMC: Debug mode enabled\n");
        fprintf(stderr, "PMC: Function list initialized: %d, count: %zu\n", 
                function_list.initialized, function_list.count);
        debug_printed = 1;
    }
    
    // Check if this function is in our list
    int in_list = is_function_in_list(info.dli_sname);
    if (debug_env) {
        fprintf(stderr, "PMC: Function %s - in list: %d\n", info.dli_sname, in_list);
    }
    
    if (!in_list) {
        return;  // Not in list, skip instrumentation
    }
    
    // Check if we have room in the stack
    if (handle_stack.depth >= MAX_HANDLE_STACK) {
        fprintf(stderr, "PMC: Warning: Handle stack overflow for function %s\n", info.dli_sname);
        return;
    }
    
    // Get CSV path from environment or use default
    const char *csv_path = getenv("PMC_CSV_PATH");
    if (!csv_path) {
        csv_path = "pmc_events.csv";
    }
    
    if (debug_env) {
        fprintf(stderr, "PMC: Starting measurement for %s\n", info.dli_sname);
    }
    
    // Use function name directly as label - duplicate checking will prevent multiple measurements
    // The label tracking mechanism in pmc_measure_begin_internal will skip if already measured
    pmc_multi_handle_t *handle = pmc_measure_begin_csv(info.dli_sname, csv_path);
    
    if (debug_env) {
        if (handle) {
            fprintf(stderr, "PMC: Measurement started successfully for %s\n", info.dli_sname);
        } else {
            fprintf(stderr, "PMC: Failed to start measurement for %s: %s\n", 
                    info.dli_sname, pmc_get_error());
        }
    }
    
    // Push handle onto stack (even if NULL, to maintain depth)
    handle_stack.handles[handle_stack.depth++] = handle;
}

/**
 * Called automatically by GCC when -finstrument-functions is used.
 * This function is called at the exit of every function.
 * 
 * @param this_fn Address of the function being exited
 * @param call_site Address of the call site (unused)
 */
void __cyg_profile_func_exit(void *this_fn, void *call_site) {
    (void)call_site;  // Unused
    (void)this_fn;    // We use the stack instead
    
    // Check if we have a handle on the stack
    if (handle_stack.depth == 0) {
        return;  // No handle, nothing to do
    }
    
    // Pop handle from stack
    handle_stack.depth--;
    pmc_multi_handle_t *handle = handle_stack.handles[handle_stack.depth];
    
    if (handle) {
        // For instrumentation mode, always export to JSON (required by collect_pmc_features.py)
        // The report parameter controls both JSON export and stdout printing
        // We always pass 1 to ensure JSON export happens
