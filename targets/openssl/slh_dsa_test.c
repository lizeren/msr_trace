                                       ...
                                       int public)
{
    ...
    pmc_multi_handle_t *pmc1 = pmc_measure_begin_csv("EVP_PKEY_CTX_new_from_name", NULL);  // NULL = use default "pmc_events.csv"
    ctx = EVP_PKEY_CTX_new_from_name(lib_ctx, alg, NULL);
    pmc_measure_end(pmc1, 1);  // 1 = auto-report results and export to JSON
    
    pmc_multi_handle_t *pmc2 = pmc_measure_begin_csv("EVP_PKEY_fromdata_init", NULL);  // NULL = use default "pmc_events.csv"
    int temp2 = EVP_PKEY_fromdata_init(ctx);
    pmc_measure_end(pmc2, 1);  // 1 = auto-report results and export to JSON
    
    pmc_multi_handle_t *pmc3 = pmc_measure_begin_csv("EVP_PKEY_fromdata", NULL);  // NULL = use default "pmc_events.csv"
    int temp3 = EVP_PKEY_fromdata(ctx, &key, selection, params);
    pmc_measure_end(pmc3, 1);  // 1 = auto-report results and export to JSON
    ...
    pmc_multi_handle_t *pmc4 = pmc_measure_begin_csv("EVP_PKEY_CTX_free", NULL);  // NULL = use default "pmc_events.csv"
    EVP_PKEY_CTX_free(ctx);
    pmc_measure_end(pmc4, 1);  // 1 = auto-report results and export to JSON
    return key;
}
...
static int slh_dsa_bad_pub_len_test(void)
{
    ...
    pmc_multi_handle_t *pmc5 = pmc_measure_begin_csv("EVP_PKEY_free", NULL);  // NULL = use default "pmc_events.csv"
    EVP_PKEY_free(pkey);
    pmc_measure_end(pmc5, 1);  // 1 = auto-report results and export to JSON
    return ret == 1;
}
...
static int slh_dsa_keygen_invalid_test(void)
{
    ...
    ctx = EVP_PKEY_CTX_new_from_name(lib_ctx, tst->name, NULL);

    pmc_multi_handle_t *pmc6 = pmc_measure_begin_csv("EVP_PKEY_keygen_init", NULL);  // NULL = use default "pmc_events.csv"
    int temp1 = EVP_PKEY_keygen_init(ctx);
    pmc_measure_end(pmc6, 1);
    ...
    pmc_multi_handle_t *pmc7 = pmc_measure_begin_csv("OSSL_PARAM_construct_octet_string", NULL);  // NULL = use default "pmc_events.csv"
    p[0] = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_SLH_DSA_SEED,
                                             seed, 97);
    pmc_measure_end(pmc7, 1);
    
    pmc_multi_handle_t *pmc8 = pmc_measure_begin_csv("OSSL_PARAM_construct_end", NULL);  // NULL = use default "pmc_events.csv"
    p[1] = OSSL_PARAM_construct_end();
    pmc_measure_end(pmc8, 1);
    ...
    pmc_multi_handle_t *pmc9 = pmc_measure_begin_csv("EVP_PKEY_CTX_set_params", NULL);  // NULL = use default "pmc_events.csv"
    int temp3 = EVP_PKEY_CTX_set_params(ctx, params);
    pmc_measure_end(pmc9, 1);

    pmc_multi_handle_t *pmc10 = pmc_measure_begin_csv("EVP_PKEY_generate", NULL);  // NULL = use default "pmc_events.csv"
    int temp2 = EVP_PKEY_generate(ctx, &pkey);
    pmc_measure_end(pmc10, 1);

    if (!TEST_int_eq(temp3, 1)
            || !TEST_int_eq(temp2, 0))
        goto err;
...
int setup_tests(void)
{
    ...
    /* Only tests with PMC measurements */
...
{
    pmc_multi_handle_t *pmc11 = pmc_measure_begin_csv("OSSL_PROVIDER_unload", NULL);  // NULL = use default "pmc_events.csv"
    OSSL_PROVIDER_unload(null_prov);
    pmc_measure_end(pmc11, 1);  // 1 = auto-report results and export to JSON
    
    OSSL_PROVIDER_unload(lib_prov);
    
    pmc_multi_handle_t *pmc12 = pmc_measure_begin_csv("OSSL_LIB_CTX_free", NULL);  // NULL = use default "pmc_events.csv"
    OSSL_LIB_CTX_free(lib_ctx);
    pmc_measure_end(pmc12, 1);  // 1 = auto-report results and export to JSON
