...
static int test_http_method(int do_get, int do_txt, int suggested_status)
{
    ...
    if(do_get) {
        pmc_multi_handle_t *pmc3 = pmc_measure_begin_csv("OSSL_HTTP_get", NULL);
        rsp = OSSL_HTTP_get(real_server ? REAL_SERVER_URL : path,
        ...
        rsp = OSSL_HTTP_transfer(NULL, NULL /* host */, NULL /* port */, path,
...
static int test_http_keep_alive(char version, int keep_alive, int kept_alive)
{
    ...
    for (res = 1, i = 1; res && i <= 2; i++) {
        pmc_multi_handle_t *pmc2 = pmc_measure_begin_csv("OSSL_HTTP_transfer", NULL);
    ...
    pmc_multi_handle_t *pmc4 = pmc_measure_begin_csv("OSSL_HTTP_close", NULL);
    OSSL_HTTP_close(rctx, res);
    pmc_measure_end(pmc4, 1);
                            ...
                            const char *exp_port, const char *exp_path)
{
    ...
    pmc_multi_handle_t *pmc1 = pmc_measure_begin_csv("OSSL_HTTP_parse_url", NULL);
    
    int temp1 = TEST_true(OSSL_HTTP_parse_url(url, &ssl, &user, &host, &port, &num,
        &path, &query, &frag));
    
        pmc_measure_end(pmc1, 1);
...
static int test_http_url_path_query_ok(const char *url, const char *exp_path_qu)
{
    ...
    int temp2 = TEST_true(OSSL_HTTP_parse_url(url, NULL, NULL, &host, NULL, NULL,
        &path, NULL, NULL));
...
static int test_http_resp_hdr_limit(size_t limit)
{
    ...
    pmc_multi_handle_t *pmc5 = pmc_measure_begin_csv("OSSL_HTTP_REQ_CTX_new", NULL);
    rctx = OSSL_HTTP_REQ_CTX_new(wbio, rbio, 8192);
    pmc_measure_end(pmc5, 1);

    if (TEST_ptr(rctx) == 0)
        goto err;
    
    pmc_multi_handle_t *pmc6 = pmc_measure_begin_csv("OSSL_HTTP_REQ_CTX_set_request_line", NULL);
