                           ...
                           RSA **retkey)
{
    ...
    if (clen != NULL)
        *clen = clentmp;
    pmc_multi_handle_t *pmc1 = pmc_measure_begin_csv("RSA_public_encrypt", NULL);  // NULL = use default "pmc_events.csv"
    ...
    pmc_multi_handle_t *pmc2 = pmc_measure_begin_csv("RSA_private_decrypt", NULL);  // NULL = use default "pmc_events.csv"
...
static int test_EVP_rsa_legacy_key(void)
{
    ...
    */
    ...
    pmc_multi_handle_t *pmc3 = pmc_measure_begin_csv("BN_bin2bn", NULL);  // NULL = use default "pmc_events.csv"
    if (!TEST_ptr((p = BN_bin2bn(p_data, sizeof(p_data), NULL))))
        ret = 0;
    pmc_measure_end(pmc3, 1);  // 1 = auto-report results and export to JSON

if (!TEST_ptr((q = BN_bin2bn(q_data, sizeof(q_data), NULL))))
    ret = 0;

if (!TEST_ptr((n = BN_bin2bn(n_data, sizeof(n_data), NULL))))
    ret = 0;

if (!TEST_ptr((d = BN_bin2bn(d_data, sizeof(d_data), NULL))))
    ret = 0;

if (!TEST_ptr((e = BN_bin2bn(e_data, sizeof(e_data), NULL))))
    ret = 0;

pmc_multi_handle_t *pmc4 = pmc_measure_begin_csv("RSA_new", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_ptr((rsa = RSA_new())))
    ret = 0;
pmc_measure_end(pmc4, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc5 = pmc_measure_begin_csv("EVP_sha256", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_ptr((md = EVP_sha256())))
    ret = 0;
pmc_measure_end(pmc5, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc6 = pmc_measure_begin_csv("EVP_MD_CTX_new", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_ptr((ctx = EVP_MD_CTX_new())))
    ret = 0;
pmc_measure_end(pmc6, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc7 = pmc_measure_begin_csv("EVP_PKEY_new", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_ptr((pkey = EVP_PKEY_new())))
    ret = 0;
pmc_measure_end(pmc7, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc8 = pmc_measure_begin_csv("RSA_set0_factors", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_true(RSA_set0_factors(rsa, p, q)))
    ret = 0;
pmc_measure_end(pmc8, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc9 = pmc_measure_begin_csv("RSA_set0_key", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_true(RSA_set0_key(rsa, n, e, d)))
    ret = 0;
pmc_measure_end(pmc9, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc10 = pmc_measure_begin_csv("EVP_PKEY_assign_RSA", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_true(EVP_PKEY_assign_RSA(pkey, rsa)))
    ret = 0;
pmc_measure_end(pmc10, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc11 = pmc_measure_begin_csv("EVP_DigestSignInit", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_true(EVP_DigestSignInit(ctx, NULL, md, NULL, pkey)))
    ret = 0;
pmc_measure_end(pmc11, 1);  // 1 = auto-report results and export to JSON

pmc_multi_handle_t *pmc12 = pmc_measure_begin_csv("EVP_DigestSign", NULL);  // NULL = use default "pmc_events.csv"
if (!TEST_true(EVP_DigestSign(ctx, sigbuf, &buflen, msgbuf, msglen)))
    ret = 0;
...
static int test_rsa_saos(void)
{
    ...
    pmc_multi_handle_t *pmc13 = pmc_measure_begin_csv("RSA_sign_ASN1_OCTET_STRING", NULL);
    int sign_result = RSA_sign_ASN1_OCTET_STRING(0, in, inlen, sig, &siglen, rsa_priv);
    pmc_measure_end(pmc13, 1);
    if (!TEST_true(sign_result))
        goto err;

    pmc_multi_handle_t *pmc14 = pmc_measure_begin_csv("RSA_verify_ASN1_OCTET_STRING", NULL);
    int verify_result = RSA_verify_ASN1_OCTET_STRING(0, in, inlen, sig, siglen, rsa_pub);
    pmc_measure_end(pmc14, 1);
