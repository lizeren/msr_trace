...
class PMCTemporalCollector:
    def __init__(self, target_binary: str, csv_file: str = "pmc_events.csv", 
        ...
        # Extract KEY=VALUE environment variable prefixes from the command
        # e.g. "LD_BIND_NOW=1 ./binary" -> env_overrides={'LD_BIND_NOW': '1'}, command=['./binary']
        self.env_overrides = {}
        actual_command_start = 0
        for elem in self.target_command:
            key, sep, value = elem.partition('=')
            if sep and key and key.replace('_', '').isalnum() and os.path.sep not in elem:
                self.env_overrides[key] = value
                actual_command_start += 1
            else:
                break
        if actual_command_start > 0:
            self.target_command = self.target_command[actual_command_start:]
            print(f"Detected env prefix(es): {self.env_overrides}")
    ...
    def collect_events(self, event_indices: List[int], num_runs: int = 5) -> int:
        ...
        for run_id in range(num_runs):
            ...
            env = os.environ.copy()
            env.update(self.env_overrides)  # Apply KEY=VALUE prefixes from the command
